...
public:
    ...
    virtual void UploadToVRAM24(int x, int y, int w, int h, const uint8_t* data) = 0;
    ...
    // CPU-side VRAM pixel access (for diagnostics). Returns null if not available.
    virtual const uint16_t* GetVRAMPixels() const { return nullptr; }
};
