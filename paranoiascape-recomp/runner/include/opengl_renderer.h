...
public:
    ...
    void UploadToVRAM24(int x, int y, int w, int h, const uint8_t* data) override;
    ...
    int GetDisplayWidth() const { return display_width_; }
    int GetDisplayHeight() const { return display_height_; }
    DrawingArea GetDrawingArea() const { return drawing_area_; }

    // CPU-side VRAM pixel access (for diagnostics)
    const uint16_t* GetVRAMPixels() const override { return vram_pixels_; }
...
private:
    ...
    uint32_t display_snapshot_ = 0;  // Snapshot of FBO after rendering (for Present to read)
    uint32_t fmv_texture_ = 0;       // FMV frame texture (decoded AVI, composited behind VRAM)
    int fmv_tex_width_ = 0;
    int fmv_tex_height_ = 0;
    ...
    uint32_t copy_shader_ = 0;       // VRAM-to-VRAM copy (and screen blit)
    uint32_t copy_24bit_shader_ = 0; // 24-bit VRAM screen blit
    ...
    int uloc_copy_discard_black_ = -1;
    int uloc_copy_24bit_source_  = -1;
    int uloc_copy_24bit_disp_x_  = -1;
    int uloc_copy_24bit_visible_w_ = -1;
