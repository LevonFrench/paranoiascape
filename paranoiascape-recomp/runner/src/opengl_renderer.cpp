...
void OpenGLRenderer::DrawTriangle(const Vertex v[3], const DrawState& state) {
    ...
    uint32_t eff_texpage_x = (state.textured && v[1].has_texpage)
...
void OpenGLRenderer::FillRectangle(int x, int y, int w, int h, uint16_t color) {
    ...
    /* During FMV playback, skip FillRect calls that would clear the FMV area
     * in the framebuffer at (0,0).  On a real PS1, the MDEC DMA writes to the
     * framebuffer during VBlank AFTER FillRect, so the FMV survives.  In our
     * synchronous pipeline, FillRect would wipe the FMV before display. */
    if (fmv_player_is_displaying() && x < 320 && y < 240) {
        return;
    }

    // Update CPU-side VRAM copy (optimized: memset for black, row fill otherwise)
    if (color == 0) {
        for (int dy = 0; dy < h; dy++) {
            memset(&vram_pixels_[(y + dy) * 1024 + x], 0, w * sizeof(uint16_t));
        }
    } else {
        for (int dy = 0; dy < h; dy++) {
            uint16_t* row = &vram_pixels_[(y + dy) * 1024 + x];
    ...
    if (opengl_initialized_ && fill_shader_ != 0) {
        ...
        if (fill_err != GL_NO_ERROR) {
            static int s_fill_errs = 0;
            if (++s_fill_errs <= 5) {
                printf("[OpenGLRenderer] GL error 0x%X in FillRect (xy=%d,%d wh=%d,%d)\n",
                       fill_err, x, y, w, h);
                fflush(stdout);
            }
        }
...
void OpenGLRenderer::UploadToVRAM(int x, int y, int w, int h, const uint16_t* data) {
    ...
    // from leaking into display_snapshot_ and double-rendering / ghosting on top
    // of the clean dedicated fmv_texture_. We still update vram_pixels_ for
    // CPU-side compatibility.
    if (x == 0 && y == 0 && fmv_player_is_displaying()) {
        for (int dy = 0; dy < h; dy++) {
            for (int dx = 0; dx < w; dx++) {
                vram_pixels_[(y + dy) * 1024 + (x + dx)] = data[dy * w + dx];
            }
        }
        vram_read_dirty_ = true;
        return;
    }
    ...
    if (!wraps_inner) {
        ...
        for (int dy = 0; dy < h; dy++)
            for (int dx = 0; dx < w; dx++)
                vram_pixels_[(y + dy) * 1024 + (x + dx)] = data[dy * w + dx];
        /* Watchpoint: track writes to TPAGE at (640,0) */
        if (x <= 640 && (x + w) > 640 && y == 0 && h > 0) {
            printf("[VRAM-WATCH] f%u UploadToVRAM(%d,%d,%d,%d) wrote to (640,0): %04X %04X %04X %04X\n",
                   g_ps1_frame, x, y, w, h,
                   vram_pixels_[0 * 1024 + 640], vram_pixels_[0 * 1024 + 641],
                   vram_pixels_[0 * 1024 + 642], vram_pixels_[0 * 1024 + 643]);
            fflush(stdout);
        }
    } else {
...
void OpenGLRenderer::CopyVRAM(int src_x, int src_y, int dst_x, int dst_y, int w, int h) {
    ...
    if (copy_err != GL_NO_ERROR) {
        static int s_copy_errs = 0;
        if (++s_copy_errs <= 5) {
            printf("[OpenGLRenderer] GL error 0x%X in CopyVRAM (src=%d,%d dst=%d,%d wh=%d,%d)\n",
                   copy_err, src_x, src_y, dst_x, dst_y, w, h);
            fflush(stdout);
        }
    }
}
...
void OpenGLRenderer::Present() {
    ...
    // Step 1b: During FMV, FlushPrimitives may have returned early (empty buffer
    // because 8-bit draws are blocked).  Force-upload the FMV frame into the FBO
    // and capture a snapshot so the FMV appears on screen.
    if (fmv_player_is_displaying()) {
        fmv_refresh_vram();   // writes FMV to vram_texture_ at (0,0)

        // No longer capture FBO (vram_texture_) to display_snapshot_ here, because
        // fmv_texture_ is drawn directly in Present() Pass 1. This prevents the
        // duplicate video frame from leaking into overlays and shimmering.
    }
    ...
    // Bind VAO and VBO for quad rendering
    glBindVertexArray(vao_);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_);

    // Use copy shader for all blits
    glUseProgram(copy_shader_);
    glDisable(GL_SCISSOR_TEST);
    glDisable(GL_BLEND);

    // Check if FMV is currently displaying
    bool fmv_active = (fmv_texture_ != 0 && fmv_player_is_displaying());

    // Diagnostic: log Present state during key transition windows
    if ((g_ps1_frame >= 850 && g_ps1_frame <= 880) || (g_ps1_frame >= 1380 && g_ps1_frame <= 1420)) {
        printf("[Present] f%u disp=(%d,%d) %dx%d fmv=%d visible_w=%d visible_h=%d\n",
               g_ps1_frame, display_area_x_, display_area_y_,
               display_width_, display_height_, fmv_active ? 1 : 0,
               visible_w, visible_h);
        fflush(stdout);
    }

    // --- Pass 1: FMV background (if active) ---
    if (fmv_active) {
        // Fullscreen quad with UV 0-1 (entire FMV texture)
        OpenGLVertex fmv_quad[6] = {};
        fmv_quad[0].x = 0.0f;    fmv_quad[0].y = 0.0f;    fmv_quad[0].u = 0.0f; fmv_quad[0].v = 0.0f;
        fmv_quad[1].x = 1024.0f; fmv_quad[1].y = 0.0f;    fmv_quad[1].u = 1.0f; fmv_quad[1].v = 0.0f;
        fmv_quad[2].x = 1024.0f; fmv_quad[2].y = 512.0f;  fmv_quad[2].u = 1.0f; fmv_quad[2].v = 1.0f;
        fmv_quad[3].x = 0.0f;    fmv_quad[3].y = 0.0f;    fmv_quad[3].u = 0.0f; fmv_quad[3].v = 0.0f;
        fmv_quad[4].x = 1024.0f; fmv_quad[4].y = 512.0f;  fmv_quad[4].u = 1.0f; fmv_quad[4].v = 1.0f;
        fmv_quad[5].x = 0.0f;    fmv_quad[5].y = 512.0f;  fmv_quad[5].u = 0.0f; fmv_quad[5].v = 1.0f;

        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(fmv_quad), fmv_quad);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, fmv_texture_);
        glUniform1i(uloc_copy_source_, 0);
        glUniform1i(uloc_copy_discard_black_, 0);  // Don't discard black in FMV
        glDrawArrays(GL_TRIANGLES, 0, 6);
    }
    ...
    if (!fmv_active || fmv_player_wants_overlay()) {
        ...
        float v1 = (display_area_y_ + visible_h) / 512.0f;
        ...
        glUniform1i(uloc_copy_source_, 0);
        glUniform1i(uloc_copy_discard_black_, fmv_active ? 1 : 0);
        glDrawArrays(GL_TRIANGLES, 0, 6);
    }
...
void OpenGLRenderer::UploadFMVFrame(int w, int h, const uint16_t* data) {
    if (!opengl_initialized_ || fmv_texture_ == 0 || !data) return;

    glBindTexture(GL_TEXTURE_2D, fmv_texture_);
    if (w != fmv_tex_width_ || h != fmv_tex_height_) {
        // Resize texture (e.g., switching from SD to HD)
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB5_A1, w, h, 0,
                     GL_RGBA, GL_UNSIGNED_SHORT_1_5_5_5_REV, data);
        fmv_tex_width_ = w;
        fmv_tex_height_ = h;
        printf("[FMV-TEX] Resized to %dx%d\n", w, h);
    } else {
...
uniform bool uDiscardBlack;  // When true, discard near-black pixels (for FMV compositing)

void main() {
    vec4 color = texture(uSourceTexture, vTexCoord);
    if (uDiscardBlack && color.r < 0.05 && color.g < 0.05 && color.b < 0.05) {
        discard;
    }
    FragColor = color;
}
        ...
        uloc_copy_discard_black_ = glGetUniformLocation(copy_shader_, "uDiscardBlack");
    }
...
bool OpenGLRenderer::InitializeVRAM() {
    ...
    // Create display snapshot texture — PS1 double-buffer emulation.
    // The game clears the FBO (FillRect) between rendering and Present(),
    // so we need to capture the rendered frame before the clear.
    glGenTextures(1, &display_snapshot_);
    glBindTexture(GL_TEXTURE_2D, display_snapshot_);
    {
        // Initialize to black (prevents uninitialized garbage on first frames)
        std::vector<uint16_t> black(1024 * 512, 0);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB5_A1, 1024, 512, 0,
                     GL_RGBA, GL_UNSIGNED_SHORT_1_5_5_5_REV, black.data());
    }
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glBindTexture(GL_TEXTURE_2D, 0);
    printf("[OpenGLRenderer] Display snapshot texture created\n");
    // Create FMV frame texture — used for compositing decoded AVI video
    // behind the game's 3D content in Present().
    glGenTextures(1, &fmv_texture_);
    glBindTexture(GL_TEXTURE_2D, fmv_texture_);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB5_A1, 320, 240, 0,
                 GL_RGBA, GL_UNSIGNED_SHORT_1_5_5_5_REV, nullptr);
    ...
    fmv_tex_height_ = 240;
    glBindTexture(GL_TEXTURE_2D, 0);
    printf("[OpenGLRenderer] FMV texture created (320x240, linear filter)\n");
...
void OpenGLRenderer::FlushPrimitives() {
    ...
    ++s_flush_total;
    if (s_flush_total <= 3) {
        bool is_tex = (vertex_buffer_[0].flags & 1);
        printf("[Flush] #%u f%u verts=%zu textured=%d area=(%d,%d)-(%d,%d)\n",
               s_flush_total, g_ps1_frame, vertex_buffer_.size(), is_tex ? 1 : 0,
    ...
    // game sprite uploads can overwrite the FMV data between fmv_player_tick()
    // and this draw call.  Re-uploading here ensures FMV is the last writer.
    fmv_refresh_vram();
    ...
    // Check for OpenGL errors with granular location info
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        static int s_flush_err = 0;
        if (++s_flush_err <= 5) {
            printf("[OpenGLRenderer] GL error 0x%X in FlushPrimitives after draw+snapshot "
                   "(draw_area=(%d,%d)-(%d,%d) disp=(%d,%d) %dx%d verts=%zu)\n",
                   error,
                   drawing_area_.x1, drawing_area_.y1, drawing_area_.x2, drawing_area_.y2,
...
void OpenGLRenderer::SaveScreenshotBMP(const char* path) {
    ...
    // Read from display_snapshot_ (same source Present() uses) rather than
    // vram_fbo_ which may have been cleared by FillRect since the last render.
    GLuint tmp_fbo;
    glGenFramebuffers(1, &tmp_fbo);
    glBindFramebuffer(GL_READ_FRAMEBUFFER, tmp_fbo);
    glFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                           GL_TEXTURE_2D, display_snapshot_, 0);
...
void OpenGLRenderer::UploadToVRAM24(int x, int y, int w, int h, const uint8_t* data) {
    if (!opengl_initialized_) return;
    int hw_width = (w * 3) / 2;
    uint16_t* temp = new uint16_t[hw_width * h];
    for (int cy = 0; cy < h; cy++) {
        for (int cx = 0; cx < w; cx += 2) {
            int src0 = (cy * w + cx) * 3;
            int src1 = src0 + 3;
            int dst = cy * hw_width + (cx / 2) * 3;
            
            // Pack two 24-bit pixels (R0, G0, B0 and R1, G1, B1) into three 16-bit halfwords (little endian)
            temp[dst]     = (uint16_t)(data[src0 + 0] | (data[src0 + 1] << 8));
            temp[dst + 1] = (uint16_t)(data[src0 + 2] | (data[src1 + 0] << 8));
            temp[dst + 2] = (uint16_t)(data[src1 + 1] | (data[src1 + 2] << 8));
        }
    }
    UploadToVRAM(x, y, hw_width, h, temp);
    delete[] temp;
}
...
// C-linkage wrapper for fmv_player.cpp to call UploadFMVFrame.
// We can't extern g_renderer directly (it's static in main_runner.cpp),
// so we use a function pointer set during initialization.
static PS1::OpenGLRenderer* s_fmv_renderer = nullptr;

extern "C" void renderer_set_fmv_renderer(void* r) {
    s_fmv_renderer = (PS1::OpenGLRenderer*)r;
}
