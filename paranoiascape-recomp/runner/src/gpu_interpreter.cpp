...
void GPUInterpreter::WriteGP0(uint32_t value) {
    static uint32_t s_gp0_words = 0;
    static uint32_t s_gp0_cmds[256] = {0}; /* count by command byte */
    static uint32_t s_last_dump_frame = 0;
    s_gp0_words++;
    
    /* Track command bytes (only for first word of each command) */
    if (state.params_received == 0 && state.params_needed == 0) {
        uint8_t cmd_byte = (uint8_t)(value >> 24);
        s_gp0_cmds[cmd_byte]++;
    }
    
    /* Dump command stats every 60 frames during Stage 1 */
    if (g_ps1_frame >= 1000 && g_ps1_frame != s_last_dump_frame && g_ps1_frame % 60 == 0) {
        s_last_dump_frame = g_ps1_frame;
        printf("[GP0-STATS] f%u total=%u | ", g_ps1_frame, s_gp0_words);
        for (int i = 0; i < 256; i++) {
            if (s_gp0_cmds[i] > 0) printf("%02X:%u ", i, s_gp0_cmds[i]);
        }
        printf("\n");
        fflush(stdout);
        /* Reset for next interval */
        memset(s_gp0_cmds, 0, sizeof(s_gp0_cmds));
    }
    ...
    while (!FIFOEmpty()) {
        ...
        while (state.params_received < state.params_needed && !FIFOEmpty()) {
            ...
            if ((state.current_command >> 29) == 0x5 && state.params_received == 3) {
                ...
                if (w > 1024) w = 1024;
                if (h > 512)  h = 512;
                printf("[CPUToVRAM] start: x=%d y=%d w=%d h=%d\n", x, y, w, h);
                VRAMTransfer& t = state.vram_transfer;
                t.active            = true;
...
void GPUInterpreter::ExecuteGP0Command() {
    ...
    if (g_ps1_frame >= 13800 && g_ps1_frame <= 13805) {
        printf("[GP0] f%d Executing cmd=0x%08X (type=%d, opcode=0x%02X)\n", g_ps1_frame, cmd, cmd_type, opcode);
        fflush(stdout);
    }
...
void GPUInterpreter::HandleGP0Polygon(const uint32_t* params) {
    ...
    if (quad && textured && g_ps1_frame >= 850 && g_ps1_frame <= 852) {
        printf("[Quad] f%d cmd=0x%02X XY0=(%d,%d) UV0=(%d,%d) XY1=(%d,%d) UV1=(%d,%d) XY2=(%d,%d) UV2=(%d,%d) XY3=(%d,%d) UV3=(%d,%d) TP=(%d,%d) depth=%d CLUT=(%d,%d)\n", 
            g_ps1_frame, (cmd >> 24) & 0xFF,
            ...
            verts[1].texpage_x, verts[1].texpage_y, verts[1].texpage_depth,
            verts[0].clut_x, verts[0].clut_y);
        /* Probe CLUT data in vram_pixels_ to verify it's populated correctly */
        if (renderer && verts[0].has_clut) {
            const uint16_t* vram = renderer->GetVRAMPixels();
            if (vram) {
                int cx = verts[0].clut_x;
                int cy = verts[0].clut_y;
                if (cy < 512 && cx + 3 < 1024) {
                    printf("  [CLUT-PROBE] (%d,%d): %04X %04X %04X %04X\n",
                           cx, cy, vram[cy*1024+cx], vram[cy*1024+cx+1],
                           vram[cy*1024+cx+2], vram[cy*1024+cx+3]);
                }
            }
        }
        fflush(stdout);
    ...
    // Render polygon(s)
    if (renderer) {
        /* FMV background filter: handled in DrawTriangle() and DrawRectangle()
         * which skip all 4-bit (depth=0) textured content. See opengl_renderer.cpp. */
...
void GPUInterpreter::HandleGP0CPUToVRAM(const uint32_t* params) {
    ...
    if (s_cpu2vram <= 20 || dst_x >= 640) {
        printf("[GP0:A0] #%u CPUToVRAM dst=(%d,%d) %dx%d (%u pixels) f%d\n",
               s_cpu2vram, dst_x, dst_y, width, height, pixel_count, g_ps1_frame);
        fflush(stdout);
    }
...
void GPUInterpreter::HandleGP0Environment(uint32_t cmd) {
    ...
    switch (opcode) {
        ...
        default:
            if (opcode != 0xFF && opcode != 0xFE && opcode != 0xFD) {
...
void GPUInterpreter::HandleDrawMode(uint32_t cmd) {
    ...
    static int s_log_e1 = 0;
    if (s_log_e1++ < 50) {
        printf("[GP0-E1] texpage:(%d,%d) depth:%d semi:%d dith:%d\n",
               state.draw_mode.texpage_x_base * 64, state.draw_mode.texpage_y_base * 256,
               state.draw_mode.texture_depth, state.draw_mode.semi_transparency, state.draw_mode.dithering);
    }
...
void GPUInterpreter::HandleTextureWindow(uint32_t param) {
    // Texture Window settings: GP0(E2h)
    // mask_x, mask_y, offset_x, offset_y in 5-bit steps of 8 pixels.
    uint32_t mx = param & 0x1F;
    uint32_t my = (param >> 5) & 0x1F;
    uint32_t ox = (param >> 10) & 0x1F;
    uint32_t oy = (param >> 15) & 0x1F;

    // PS1 texture window applies to 8-bit UVs (0-255).
    // The mask allows bits through where the mask register is 0.
    // Bits where mask register is 1 are forced to the offset register.
    // Minimum step size is 8 pixels, so lower 3 bits are always allowed (1).
    state.texture_window.mask_x = ~(mx * 8) & 0xFF;
    state.texture_window.mask_y = ~(my * 8) & 0xFF;
    state.texture_window.offset_x = (ox & mx) * 8;
    state.texture_window.offset_y = (oy & my) * 8;
...
void GPUInterpreter::HandleDrawingAreaTopLeft(uint32_t cmd) {
    ...
    if (g_ps1_frame >= 13500 && g_ps1_frame <= 14000) {
        printf("[GP0:E3] f%u DrawArea TL=(%d,%d)\n", g_ps1_frame, state.drawing_area.x1, state.drawing_area.y1);
    }
...
void GPUInterpreter::HandleDrawingAreaBottomRight(uint32_t cmd) {
    ...
    if (g_ps1_frame >= 13500 && g_ps1_frame <= 14000) {
        printf("[GP0:E4] f%u DrawArea BR=(%d,%d)\n", g_ps1_frame, state.drawing_area.x2, state.drawing_area.y2);
    }
...
void GPUInterpreter::HandleDrawingOffset(uint32_t cmd) {
    ...
    if (g_ps1_frame >= 13500 && g_ps1_frame <= 14000) {
        printf("[GP0:E5] f%u DrawOffset=(%d,%d)\n", g_ps1_frame, x, y);
    }
...
void GPUInterpreter::HandleGP1DisplayAreaStart(uint32_t param) {
    ...
    if (s_gp1_05_count++ < 20) {
        printf("[GP1:05] #%u f%u display_area=(%d,%d)\n", s_gp1_05_count-1, g_ps1_frame,
