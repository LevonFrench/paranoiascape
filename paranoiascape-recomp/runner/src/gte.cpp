...
extern "C" void gte_execute(CPUState* cpu, uint32_t cmd) {
    ...
    if ((func == 0x01 || func == 0x30) && (cpu->gte_ctrl[31] & 0x80000000u)) {
        ...
        /* Reset counter every 300 frames to see if errors persist */
        if (g_ps1_frame > s_last_frame + 300) {
            s_flag_err = 0;
        }
        s_last_frame = g_ps1_frame;
        if (++s_flag_err <= 5) {
            printf("[GTE-FLAG] RTPS err #%d f%u FLAG=0x%08X "
                   "V0=(%d,%d,%d) TR=(%d,%d,%d) H=%u SZ3=%u "
                   "MAC=(%d,%d,%d) IR=(%d,%d,%d) SXY=0x%08X ra=0x%08X\n",
                   ...
                   cpu->gte_data[14], cpu->ra);
            if (s_flag_err == 1) {
                printf("  [GTE-RT] [%d %d %d] [%d %d %d] [%d %d %d]\n",
                       gte.RT[0][0], gte.RT[0][1], gte.RT[0][2],
                       gte.RT[1][0], gte.RT[1][1], gte.RT[1][2],
                       gte.RT[2][0], gte.RT[2][1], gte.RT[2][2]);
                printf("  [GTE-CTRL] ctrl5=0x%08X ctrl6=0x%08X ctrl7=0x%08X\n",
                       cpu->gte_ctrl[5], cpu->gte_ctrl[6], cpu->gte_ctrl[7]);
            }
            fflush(stdout);
        }
    }
    /* Also log first 10 successful RTPS to see healthy TR values */
    if ((func == 0x01 || func == 0x30) && !(cpu->gte_ctrl[31] & 0x80000000u)) {
        static int s_ok = 0;
        if (++s_ok <= 10) {
            printf("[GTE-OK] RTPS #%d f%u TR=(%d,%d,%d) H=%u ra=0x%08X\n",
                   s_ok, g_ps1_frame,
                   gte.TR[0], gte.TR[1], gte.TR[2],
                   gte.H, cpu->ra);
            fflush(stdout);
