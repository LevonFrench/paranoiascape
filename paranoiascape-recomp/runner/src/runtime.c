...
/* Safety guard loop counter for func_80073BC4 */
int g_c3bc4_loop_cnt = 0;
...
static uint16_t get_vblank_count(void) {
    ...
    return (uint16_t)(elapsed * 60.0);
}

static void update_vblank_counters(void) {
    static LARGE_INTEGER freq = {0};
    ...
    uint16_t ticks = (uint16_t)(elapsed * 60.0);
    if (ticks > 0) {
        uint16_t cnt_1e8, cnt_1f6;
        memcpy(&cnt_1e8, &g_scratch[0x1E8], 2);
        memcpy(&cnt_1f6, &g_scratch[0x1F6], 2);
        cnt_1e8 += ticks;
        cnt_1f6 += ticks;
        memcpy(&g_scratch[0x1E8], &cnt_1e8, 2);
        memcpy(&g_scratch[0x1F6], &cnt_1f6, 2);
        
        double tick_duration = 1.0 / 60.0;
        last.QuadPart += (LONGLONG)(ticks * tick_duration * freq.QuadPart);
    }
}
    ...
    if (phys == 0x1F8010A8u) {
        ...
        if ((value & 0x01000000u) && sync == 1u && dir == 1u) {
            ...
            for (int ll_limit = 0; ll_limit < 65536; ll_limit++) {
                if (ptr < 0x80010000u) break;  /* Bounds check: skip kernel/BIOS area */
                ...
                /* Move hare 2 steps */
                int hare_ended = 0;
                uint8_t* ph_h1 = addr_ptr(hare);
                if (ph_h1) {
                    uint32_t hdr_h1; memcpy(&hdr_h1, ph_h1, 4);
                    uint32_t nxt_h1 = hdr_h1 & 0xFFFFFFu;
                    if (nxt_h1 != 0xFFFFFFu && nxt_h1 != 0u) {
                        hare = nxt_h1 | 0x80000000u;
                        uint8_t* ph_h2 = addr_ptr(hare);
                        if (ph_h2) {
                            uint32_t hdr_h2; memcpy(&hdr_h2, ph_h2, 4);
                            uint32_t nxt_h2 = hdr_h2 & 0xFFFFFFu;
                            if (nxt_h2 != 0xFFFFFFu && nxt_h2 != 0u) {
                                hare = nxt_h2 | 0x80000000u;
                            } else {
                                hare_ended = 1;
                            }
                        } else {
                ...
                if (!hare_ended && ptr == hare) {
                    printf("[DMA2-LL] CYCLE DETECTED! ptr=0x%08X hare=0x%08X at ll_limit=%d ra=0x%08X\n", ptr, hare, ll_limit, g_diag_cpu ? g_diag_cpu->ra : 0);
                    fflush(stdout);
                    break;
                }
            }
...
static uint16_t read_half(uint32_t addr) {
    ...
    if (addr == 0x1F8001E8u) {
        uint16_t old_val;
        memcpy(&old_val, &g_scratch[0x1E8], 2);
        update_vblank_counters();
        uint16_t new_val;
        memcpy(&new_val, &g_scratch[0x1E8], 2);
        if (new_val == old_val) {
            Sleep(1);
        }
        v = new_val;
...
static uint8_t read_byte(uint32_t addr) {
    ...
    if (phys == 0x1F801040u) {
        ...
        if (s_sio_state == 0) return 0xFF;
        if (s_sio_state == 1) return 0xFF;
        if (s_sio_state == 2) return 0x41;
        if (s_sio_state == 3) return 0x5A;
        if (s_sio_state == 4) return (uint8_t)(pad & 0xFF);
...
static int is_compiled_addr(uint32_t addr) {
    if (addr == 0x8006B458u || addr == 0x8006B58Cu || addr == 0x8006B924u || addr == 0x8006BEB8u ||
        addr == 0x8006B964u || addr == 0x8006B934u || addr == 0x8006B7D8u || addr == 0x8006B798u) {
        return 0;
    }
    return (addr >= 0x8000F000u && addr < 0x80098000u);
}
    ...
    for (guard = 0; guard < 10000; guard++) {
        ...
        if (is_link) {
            ...
            if (target == pc + 8) {
                ...
                if (psx_dispatch_compiled(cpu, target)) {
                    return;
                }
                pc = target;
            } else {
...
static int g_in_irq_dispatch = 0;
static void psx_dispatch_int_chain(CPUState *cpu, uint32_t priority) {
    if (g_in_irq_dispatch) return;
    g_in_irq_dispatch = 1;
    
    uint32_t entry = g_int_chains[priority & 3];
    int limit = 16;
    while (entry && --limit >= 0) {
        uint32_t ephys = entry & 0x1FFFFFFFu;
        if (ephys + 8 > sizeof(g_ram)) break;
        
        uint32_t verifier = 0;
        memcpy(&verifier, &g_ram[ephys + 8], 4);
        uint32_t handler = 0;
        memcpy(&handler, &g_ram[ephys + 4], 4);
        
        if (verifier) {
            cpu->v1 = entry;
            call_by_address(cpu, verifier);
            if (cpu->v0 == 0) {
                uint32_t next = 0;
...
void call_by_address(CPUState* cpu, uint32_t addr) {
    /* Safety guard to break out of buggy loop in func_80073BC4 */
    extern int g_c3bc4_loop_cnt;
    if (cpu->ra >= 0x80073BC4u && cpu->ra < 0x80074450u) {
        g_c3bc4_loop_cnt++;
        static int s_calls = 0;
        if (s_calls++ < 150) {
            printf("[func_80073BC4-CALL] target=0x%08X ra=0x%08X loop_cnt=%d\n", addr, cpu->ra, g_c3bc4_loop_cnt);
            fflush(stdout);
        }
        if (g_c3bc4_loop_cnt > 5000) {
            static int s_logged = 0;
            if (s_logged++ < 10) {
                printf("[func_80073BC4] Loop iteration count %d exceeded 5000 in call_by_address! Breaking loop (s1 was %d, s0 was 0x%08X).\n",
                       g_c3bc4_loop_cnt, (int)cpu->s1, cpu->s0);
                fflush(stdout);
            }
            cpu->s1 = 0; // force break loop
        }
    }
    ...
    if (addr != 0xA0u && addr != 0xB0u && addr != 0xC0u &&
        (kseg0 < 0x8000F000u || kseg0 > 0x801FFFFFu)) {
                ...
                if (cpu->a0) {
                    ...
                    if (strstr(s, "non supported code") != NULL) {
                        ...
                        if (s_gpu_code_warns++ < 20) {
                            printf("[BIOS printf] GPU CODE 0x%02X not assigned. caller_ra=0x%08X (total=%u)\n", cpu->a1, cpu->ra, s_gpu_code_warns);
                        }
                    } else {
    ...
    if (kseg0 >= 0x8000F000u && kseg0 < 0x80098000u) {
...
void inject_pad_state(void) {
    static uint16_t s_last_pad = 0xFFFF;
    extern uint16_t g_pad1_state;
    uint16_t pad = ~g_pad1_state;
    ...
    if (override != -1 && override != 0) {
        /* If overriding, hold 'pressed' active as well, in case the game misses the 1-frame edge */
        pressed = ~(uint16_t)override;
    }
    
    uint16_t released = ~(pad & ~s_last_pad); /* Active LOW! */
    s_last_pad = pad;

    memcpy(&g_ram[0x13E5EC], &pad, 2);
    memcpy(&g_ram[0x13E5EE], &pressed, 2);
    memcpy(&g_ram[0x13E5F0], &released, 2);

    /* Write to the raw pad buffer at 0x12F9C8 so func_80019304 registers it */
    g_ram[0x12F9C8] = 0;    // Connected
    g_ram[0x12F9C9] = 0x41; // Standard controller type
    g_ram[0x12F9CA] = pad & 0xFF; // Buttons low byte (active-low)
    g_ram[0x12F9CB] = (pad >> 8) & 0xFF; // Buttons high byte (active-low)
    /* Diagnostic: log whenever override is active */
    if (override != -1 && override != 0) {
        static int s_inj_log = 0;
        if (s_inj_log++ < 30) {
            printf("[INJECT-PAD] f%u override=0x%04X pad=0x%04X pressed=0x%04X released=0x%04X\n",
                   g_ps1_frame, override, pad, pressed, released);
            fflush(stdout);
        }
    }
    if (override != -1 && override != 0) {
        if ((override & 0x0008) != 0) {
            static int printed = 0;
            if (printed++ % 60 == 0) {
                printf("[PAD] START injected to 13E5EC\n");
                fflush(stdout);
            }
        }
        ...
        uint32_t sm;
        memcpy(&sm, &g_ram[0x8580], 4);
        if (sm == 0x08 && (override & 0x4000)) {
            static int s_forced = 0;
            if (s_forced++ <= 5) {
                extern uint32_t g_ps1_frame;
                printf("[GLOBAL-HACK] CROSS pressed at f%u! Forcing sm=0x00\n", g_ps1_frame);
                fflush(stdout);
            }
            uint32_t next = 0x00;
            memcpy(&g_ram[0x8580], &next, 4);
        }
    }
}
...
int psx_override_dispatch(CPUState* cpu, uint32_t addr) {
     ...
     * The master precomputed table is in .rodata at 0x800A0438 (physical 0xA0438).
     * On real PS1, InitGeom()/GsInitCoord2param() builds this at startup.
     * Our recomp skips that init. The regions overlap in g_ram so we must
     * copy via a temp buffer to avoid corrupting the source during the write.
     * UPDATE: The master table in g_ram is ALSO corrupted by early boot code.
     * Fix: compute the table from scratch using the PsyQ 4.12 fixed-point format.
     * Each entry is a packed {cos(high16), sin(low16)} in 4.12 fixed-point. */
    {
        static int s_sincos_init = 0;
        if (!s_sincos_init) {
            s_sincos_init = 1;
            uint32_t dst = 0x9FC38;   /* physical offset of BSS rsin_tbl */
            uint32_t table_entries = 4096;
            uint32_t table_size = table_entries * 4;
            if (dst + table_size <= sizeof(g_ram)) {
                for (uint32_t i = 0; i < table_entries; i++) {
                    double angle = (double)i * 2.0 * 3.14159265358979323846 / (double)table_entries;
                    int16_t s = (int16_t)(sin(angle) * 4096.0 + 0.5);
                    int16_t c = (int16_t)(cos(angle) * 4096.0 + 0.5);
                    /* Handle negative rounding */
                    if (sin(angle) < 0 && s > 0) s = (int16_t)(sin(angle) * 4096.0 - 0.5);
                    if (cos(angle) < 0 && c > 0) c = (int16_t)(cos(angle) * 4096.0 - 0.5);
                    uint32_t packed = ((uint32_t)(uint16_t)c << 16) | (uint32_t)(uint16_t)s;
                    memcpy(&g_ram[dst + i * 4], &packed, 4);
                }
                /* Verify */
                uint32_t check = 0;
                memcpy(&check, &g_ram[dst], 4);
                printf("[GTE-FIX] Built sin/cos table at 0x8009FC38 (%u entries) "
                       "tbl[0]=0x%08X (expected 0x10000000)\n",
                       table_entries, check);
                fflush(stdout);
            }
        }
    ...
    if (addr == 0x80055F44u) {
        ...
        if (gp436 == 0) {
            ...
            if (!s_in_vblank && elapsed >= 1.0 / 60.0) {
                ...
                /* Back up all MIPS registers + PC, HI, LO to guarantee zero side effects on the caller */
    ...
    if (addr == 0x80050EA8u) { cpu->v0 = 1; return 1; }
    if (addr == 0x80050E68u) { cpu->v0 = 1; return 1; }
    if (addr == 0x80051A14u) { 
        extern void psx_cdrom_populate_file_table(uint8_t*, uint32_t);
        psx_cdrom_populate_file_table(g_ram, 0x00091FB4u);
        cpu->v0 = 1; 
        return 1; 
    } /* The REAL high-level CdInit loop */
    if (addr == 0x80051B50u) { cpu->v0 = 1; return 1; } /* The inner CdInit */
    if (addr == 0x80051F30u) { cpu->v0 = 1; return 1; } /* CdControl ? */
    if (addr == 0x800517E8u) { cpu->v0 = 1; return 1; } /* wrapper loop ? */

    /* Boot initialization chain trace */
    if (addr == 0x800137C4u) {
        printf("[BOOT-TRACE] func_800137C4 (file loader init) called! ra=0x%08X f%u\n", cpu->ra, g_ps1_frame);
        fflush(stdout);
        return 0; /* let it run natively */
    }
    if (addr == 0x80013DCCu) {
        printf("[BOOT-TRACE] func_80013DCC (asset loader) called! ra=0x%08X f%u\n", cpu->ra, g_ps1_frame);
        fflush(stdout);
        return 0;
    }
    if (addr == 0x80019704u) {
        printf("[BOOT-TRACE] func_80019704 (primary CD loader) called! a0=0x%08X ra=0x%08X f%u\n", cpu->a0, cpu->ra, g_ps1_frame);
        fflush(stdout);
        return 0;
    }
    if (addr == 0x800130D4u) {
        static uint32_t s_tick = 0;
        if (++s_tick <= 5 || s_tick % 200 == 0) {
            uint8_t gp321 = g_ram[(cpu->gp & 0x1FFFFF) + 321];
            printf("[MAIN-TICK] #%u f%u gp+321=%u gp+601=%u gp+829=%u\n", s_tick, g_ps1_frame,
                   gp321, g_ram[(cpu->gp & 0x1FFFFF) + 601], g_ram[(cpu->gp & 0x1FFFFF) + 829]);
            fflush(stdout);
        }
        return 0;
    }
    ...
    /* [BOOT] func_8001E468 — secondary file loader (FMV streaming).
     * This function sets up CD streaming for LOGO.STR / MOVTEST.STR FMVs.
     * The native MDEC decode path runs in the interpreter and uploads black
     * frames because CD sector data arrives as zeros through hardware stubs.
     * The resulting CD task hangs forever, blocking the boot sequence.
     * Fix: Extract the target file LBA, initialize our asynchronous 
     * fmv_player, and return 1 to bypass the blocking MIPS loop.
     * The engine will continue to the Title Screen and VSync will pump 
     * the asynchronous FMV decode. */
    if (addr == 0x8001E468u) {
        uint32_t table_idx = cpu->read_word(cpu->gp + 900);
        uint32_t entry_addr = (0x80091FB4u + table_idx * 12) & 0x1FFFFF;
        
        uint32_t name_ptr = 0, lba = 0;
        if (entry_addr + 8 <= sizeof(g_ram)) {
            memcpy(&name_ptr, &g_ram[entry_addr], 4);
            memcpy(&lba, &g_ram[entry_addr + 4], 4);
        }
        
        char filename[64] = {0};
        uint32_t n_addr = name_ptr & 0x1FFFFF;
        if (name_ptr != 0 && n_addr < sizeof(g_ram)) {
            for (int i = 0; i < 63 && n_addr < sizeof(g_ram); i++) {
                filename[i] = (char)g_ram[n_addr++];
                if (filename[i] == 0) break;
            }
        }
        
        printf("[FMV BYPASS] func_8001E468 called for '%s', LBA=%u\n", filename, lba);
        fflush(stdout);
        
        extern void fmv_player_seek(uint32_t lba, const char* filename);
        fmv_player_seek(lba, filename);
        
        /* Unblock any secondary state machines waiting for this to finish */
        cpu->write_word(cpu->gp + 1060, 1);
        cpu->write_word(cpu->gp + 1076, 0); 
        
        return 1;
    }

    /* [TITLE-FIX] Force osm=15 (title screen) at frame 120.
     ...
     * TODO: Fix FMV completion to naturally advance osm 1→15. */
    {
        ...
        if (overlay_sm == 1 && g_ps1_frame >= 120) {
            ...
            static int s_osm_forced = 0;
            if (!s_osm_forced) {
                printf("[TITLE-FIX] Forced osm=1 -> osm=15 at frame %u\n", g_ps1_frame);
                fflush(stdout);
                s_osm_forced = 1;
            }
        }
     ...
     * looks up sin/cos from table at 0x8009FC38. */
    if (addr == 0x80056514u) {
        static int s_rotmx = 0;
        if (++s_rotmx <= 10) {
            uint32_t ptr = cpu->a0 & 0x1FFFFFu;
            if (ptr + 6 <= sizeof(g_ram)) {
                int16_t rx, ry, rz;
                memcpy(&rx, &g_ram[ptr + 0], 2);
                memcpy(&ry, &g_ram[ptr + 2], 2);
                memcpy(&rz, &g_ram[ptr + 4], 2);
                /* Verify sin/cos table at 0x8009FC38 */
                uint32_t tbl = 0x9FC38;
                uint32_t entry0_raw = 0;
                memcpy(&entry0_raw, &g_ram[tbl], 4);
                uint32_t entry1_raw = 0;
                memcpy(&entry1_raw, &g_ram[tbl + 4], 4);
                printf("[ROTMX] #%d f%u a0=0x%08X angles=(%d,%d,%d) "
                       "tbl[0]=0x%08X tbl[1]=0x%08X a1=0x%08X ra=0x%08X\n",
                       s_rotmx, g_ps1_frame, cpu->a0,
                       rx, ry, rz, entry0_raw, entry1_raw,
                       cpu->a1, cpu->ra);
                fflush(stdout);
            }
        }
        return 0; /* let compiled code execute */
    }

    /* [GTE-MATRIX-TRACE] Log SetRotMatrix / SetTransMatrix values to find zero-RT source.
     * func_800561D4 = SetRotMatrix — writes gte_ctrl[0-4] from a0[0..16]
     * func_80056234 = SetTransMatrix — writes gte_ctrl[5-7] from a0[20..28]
     * We DON'T intercept — we just log and return 0 to let the compiled code run. */
    if (addr == 0x800561D4u) {
        static int s_setrot = 0;
        if (++s_setrot <= 30) {
            uint32_t ptr = cpu->a0 & 0x1FFFFFu;
            if (ptr + 20 <= sizeof(g_ram)) {
                uint32_t w0, w1, w2, w3, w4;
                memcpy(&w0, &g_ram[ptr +  0], 4);
                memcpy(&w1, &g_ram[ptr +  4], 4);
                memcpy(&w2, &g_ram[ptr +  8], 4);
                memcpy(&w3, &g_ram[ptr + 12], 4);
                memcpy(&w4, &g_ram[ptr + 16], 4);
                int all_zero = (w0 == 0 && w1 == 0 && w2 == 0 && w3 == 0 && w4 == 0);
                printf("[SETROT] #%d f%u a0=0x%08X rt=[0x%08X 0x%08X 0x%08X 0x%08X 0x%08X]%s ra=0x%08X\n",
                       s_setrot, g_ps1_frame, cpu->a0,
                       w0, w1, w2, w3, w4,
                       all_zero ? " ALL-ZERO" : "",
                       cpu->ra);
                fflush(stdout);
        ...
        return 0; /* let compiled code execute normally */
    }
    if (addr == 0x80056234u) {
        static int s_settrans = 0;
        if (++s_settrans <= 30) {
            uint32_t ptr = cpu->a0 & 0x1FFFFFu;
            if (ptr + 32 <= sizeof(g_ram)) {
                int32_t tr0, tr1, tr2;
                memcpy(&tr0, &g_ram[ptr + 20], 4);
                memcpy(&tr1, &g_ram[ptr + 24], 4);
                memcpy(&tr2, &g_ram[ptr + 28], 4);
                printf("[SETTRANS] #%d f%u a0=0x%08X tr=(%d,%d,%d) ra=0x%08X\n",
                       s_settrans, g_ps1_frame, cpu->a0,
                       tr0, tr1, tr2, cpu->ra);
                fflush(stdout);
            }
        }
    ...
    /* [CDROM] Sync bypasses */
    if (addr == 0x80052800u || addr == 0x80052A80u || addr == 0x80052AE4u || 
        addr == 0x80052D48u || addr == 0x80052D50u || 
        addr == 0x80052F1Cu || addr == 0x80053618u) {
        if (cpu->a1) {
            uint32_t res_ptr = cpu->a1 & 0x1FFFFFFF;
            if (res_ptr < sizeof(g_ram)) {
                g_ram[res_ptr] = 0x02; /* STAT_STANDBY */
            }
        }
        /* For CdReadSync/CdSync (0x80052D48/50), return 0 to indicate 0 sectors remaining / success.
         * Returning 2 causes infinite waiting. */
        cpu->v0 = 0; 
        return 1;
    }

    /* [CDROM/Task] Poll Deadlock Fix 
     * The engine spins in 0x80055314 waiting for a task status to change from 1 -> 2.
     * The task is driven by a VBlank callback registered via InterruptCallback(1, cb).
     * Since the engine is synchronous and not calling VSync, we must manually pump 
     * the registered kernel VBlank callback to fire callbacks. */
    static uint32_t s_vblank_cb = 0;
    if (addr == 0x80050D74u) {
        if (cpu->a0 == 1) { // VBlank
            s_vblank_cb = cpu->a1;
            printf("[InterruptCallback] Registered VBlank cb=0x%08X\n", s_vblank_cb);
            fflush(stdout);
        }
        return 0; // Let original MIPS execute
    }

    if (addr == 0x80055314u) {
        if (s_vblank_cb != 0) {
            extern void call_by_address(CPUState*, uint32_t);
    ...
    /* [VSYNC] Intercepted 0x8007D738 (VSync/DrawSync spinloop)
     * Set g_frame_flip_running so psx_present_frame swaps buffers,
     * then present and tick FMV. Return 0 so MIPS code still runs. */
    if (addr == 0x8007D738u) {
    ...
    /* [VSYNC] func_80050B14 — PsyQ VSync(mode).
     * On real PS1 this busy-waits on GPU FIFO registers (block_80050B28)
     * then calls func_80050C7C to wait for VBlank counter >= target.
     * We bypass the entire function: simulate VBlank ticks at 60Hz,
     * present frames, and return the counter value in v0.
     *
     * Modes: a0 < 0 → return current counter (no wait)
     *        a0 == 0 → wait for next VBlank (counter changes)
     *        a0 == 1 → wait for current + 1
     *        a0 > 1  → wait for current + a0 */
    if (addr == 0x8008E434u) {
        static int e434_cnt = 0;
        if (e434_cnt++ % 60 == 0) {
            printf("[HOOK] 8008E434 called!\n");
            fflush(stdout);
        }
    }

    if (addr == 0x8007DEB0u) {
        static int drawsync_cnt = 0;
        if (drawsync_cnt++ % 60 == 0) {
            printf("[HOOK] 8007DEB0 (DrawSync) called! ra=0x%08X\n", cpu->ra);
            fflush(stdout);
        }
    }

    /* [FMV-SKIP] REMOVED — was killing FMV playback after frame 60.
     * The force-skip was needed when func_8001E468 was skipped entirely.
     * Now that E468 initializes fmv_player_seek(), this flag would kill
    ...
    if (addr == 0x80050B14u || addr == 0x80050B04u) {
        ...
        if ((int32_t)cpu->a0 < 0) {
            /* Mode < 0: return current counter, no wait.
             * However, if the game spins on VSync(-1), we must pump VBlank! */

            /* Always inject pad state — the game's VSync(-1) spin loop
             * reads pad data between ticks, so we must update it on every call. */
            extern void inject_pad_state(void);
            inject_pad_state();
            ...
            if (elapsed >= (freq.QuadPart / 60)) {
                last = now;
                uint32_t old_ra = cpu->ra;
                cpu->v0 = *(uint32_t*)(g_ram + 0x9F278);  /* func_80051434 expects v0 = current counter */
                extern void call_by_address(CPUState* cpu, uint32_t addr);
                call_by_address(cpu, 0x80051434u);
                
                /* We cannot call func_8008D60C manually here because it loops infinitely
                 * waiting for SIO hardware interrupts. Instead, we intercept it and
                 * populate 0x13E5EC in psx_override_dispatch. */
                cpu->ra = old_ra;
                ...
                /* Check debug server input and force state transition if START is pressed */
                extern void inject_pad_state(void);
                inject_pad_state();
                
                current = *(uint32_t*)(g_ram + 0x9F278);
            }
            cpu->v0 = current;
            return 1;
        }
        ...
        if (cpu->a0 == 0) {
            ...
            target = current + cpu->a0;
        }

        /* Wait loop: tick VBlank at 60Hz until counter >= target */
        while (1) {
            current = *(uint32_t*)(g_ram + 0x9F278);
            if ((int32_t)current >= (int32_t)target) break;

            /* Throttle to 60Hz real time */
            static LARGE_INTEGER freq = {0};
            static LARGE_INTEGER last = {0};
            if (freq.QuadPart == 0) {
                QueryPerformanceFrequency(&freq);
            ...
            /* Trigger VBlank callbacks (increments counter at 0x8009F278) */
            uint32_t old_ra = cpu->ra;
            cpu->v0 = *(uint32_t*)(g_ram + 0x9F278);  /* func_80051434 expects v0 = current counter */
            extern void call_by_address(CPUState* cpu, uint32_t addr);
            call_by_address(cpu, 0x80051434u);
    ...
    /* [TMD-INTERP] Stop interpreter at native return address */
    if (addr == 0x800160B4u) {
        return 1;
    }

    /* [CDROM] Bypass for CdSearchFile
     * PsyQ signature: CdSearchFile(CdlFILE *fp, char *name)
     *   a0 = CdlFILE* fp  (output: receives pos/size)
     *   a1 = char* name   (input: filename to search) */
    if (addr == 0x80053A04u) {
        uint32_t cdl_file_ptr = cpu->a0 & 0x1FFFFF;  /* a0 = CdlFILE output */
        char* filename = (char*)&g_ram[cpu->a1 & 0x1FFFFF];  /* a1 = filename */
        extern int psx_cdrom_search_file(const char*, uint32_t*, uint32_t*);
        
        char clean_name[64];
        int j = 0;
        for (int i = 0; filename[i] && j < 63; i++) {
            if (filename[i] == ';') break; /* strip ISO9660 version suffix */
        ...
        if (psx_cdrom_search_file(cn, &pos, &size)) {
            memcpy(&g_ram[cdl_file_ptr], &pos, 4);
            memcpy(&g_ram[cdl_file_ptr + 4], &size, 4);
            cpu->v0 = 1;
            static int s_cdsf_ok = 0;
            if (++s_cdsf_ok <= 20) {
                printf("[CdSearchFile] OK '%s' pos=%u size=%u\n", cn, pos, size);
                fflush(stdout);
            }
        } else {
            cpu->v0 = 0;
            static int s_cdsf_fail = 0;
            if (++s_cdsf_fail <= 20) {
                printf("[CdSearchFile] FAIL a1=0x%08X '%s' (raw: '%.32s')\n", cpu->a1, cn, filename);
                fflush(stdout);
            }
        }
        return 1;
    ...
    /* [CDROM] Bypass for LoadTask (func_800197C8)
     * This function is NOT just a queue — it calls CdControl(SetLoc),
     * CdRead(), then spins on CdReadSync() until the read completes.
     * We must read the file here because the native CdRead/CdReadSync
     * polling requires CD hardware register emulation we don't have. */
    if (addr == 0x800197C8u) {
        uint32_t task_ptr = (cpu->a0 + (cpu->a1 * 32)) & 0x1FFFFF;
        uint32_t dest_buf = 0; memcpy(&dest_buf, &g_ram[task_ptr + 4], 4);
        uint32_t pos = 0; memcpy(&pos, &g_ram[task_ptr + 8], 4);
        uint32_t size = 0; memcpy(&size, &g_ram[task_ptr + 12], 4);
        
        printf("[LoadTask] task_ptr=0x%08X dest=0x%08X pos=%u size=%u\n", task_ptr, dest_buf, pos, size); fflush(stdout);
        
        if (size > 0) {
            extern int psx_cdrom_read_file_by_lba(uint32_t, uint32_t, uint8_t*);
            psx_cdrom_read_file_by_lba(pos, size, &g_ram[dest_buf & 0x1FFFFF]);
            printf("[LoadTask] First 8 bytes: %02X %02X %02X %02X %02X %02X %02X %02X\n",
                   g_ram[(dest_buf & 0x1FFFFF) + 0], g_ram[(dest_buf & 0x1FFFFF) + 1],
                   g_ram[(dest_buf & 0x1FFFFF) + 2], g_ram[(dest_buf & 0x1FFFFF) + 3],
                   g_ram[(dest_buf & 0x1FFFFF) + 4], g_ram[(dest_buf & 0x1FFFFF) + 5],
                   g_ram[(dest_buf & 0x1FFFFF) + 6], g_ram[(dest_buf & 0x1FFFFF) + 7]);
            if ((dest_buf & 0x1FFFFF) == 0x1D0000) {
                printf("[LoadTask] 0x801EE560: %02X %02X %02X %02X\n",
                       g_ram[0x1EE560], g_ram[0x1EE561], g_ram[0x1EE562], g_ram[0x1EE563]);
            }
            fflush(stdout);
        }
        cpu->v0 = 0; /* Success */
    ...
    /* [CDROM] Secondary load bypass */
    if (addr == 0x80081038u) {
        if (cpu->ra == 0x8001E588u) {
            uint32_t gp = cpu->gp & 0x1FFFFF;
            uint32_t val = 1;
            memcpy(&g_ram[gp + 1060], &val, 4);
            cpu->v0 = 1;
        }
        return 1;
    }
    ...
    /* ---- func_80073BC4 diagnostic trace hook ---- */
    if (addr == 0x80073BC4u) {
        extern int g_c3bc4_loop_cnt;
        g_c3bc4_loop_cnt = 0;
        uint32_t a0_val = cpu->a0;
        uint32_t a1_val = cpu->a1;
        uint32_t a2_val = cpu->a2;
        uint32_t a3_val = cpu->a3;
        ...
        if (phys_a0 + 12 <= 0x200000u) {
            ...
            if (phys_struct + 24 <= 0x200000u) {
                ...
                /* Print jump table once */
                static int s_table_printed = 0;
                if (!s_table_printed) {
                    s_table_printed = 1;
                    uint32_t tbl_addr = 0x800127A4u & 0x1FFFFFFFu;
                    printf("[JUMP-TABLE] Dumping jump table at 0x800127A4:\n");
                    for (int i = 0; i < 30; i++) {
                        uint32_t val = 0;
                        memcpy(&val, &g_ram[tbl_addr + i*4], 4);
                ...
                if (s1_val > 0) {
                    ...
                    if (phys_s0 + 4 > 0x200000u) {
                        invalid = 1;
                        reason = "s0 pointer out of bounds";
                    } else {
                        uint8_t prim_code = g_ram[phys_s0 + 3];
                        uint8_t a1 = prim_code & 0xFD;
                        int32_t v1 = (int32_t)a1 - 32;
                        
                        if (v1 < 0 || v1 >= 30) {
                            invalid = 1;
                            reason = "unsupported primitive code (potential uninitialized RAM)";
                        } else {
                            uint16_t size_val = 0;
                            memcpy(&size_val, &g_ram[phys_s0], 2);
                            if (size_val == 0) {
                                invalid = 1;
                                reason = "zero element size";
                            }
                        }
                    }
                    ...
                    if (invalid) {
                        uint32_t zero_val = 0;
                        memcpy(&g_ram[phys_struct + 20], &zero_val, 4);
                        printf("[func_80073BC4] Warning: Loop safety guard triggered! s0=0x%08X s1=%d reason='%s'. Clearing s1 to 0 to prevent infinite loop.\n",
                               s0_val, (int)s1_val, reason);
                        fflush(stdout);
                        s1_val = 0; /* Update local copy for trace printing */
                    }
                }
            }
        }
        
        static int s_c3bc4_cnt = 0;
        if (s_c3bc4_cnt++ < 100) {
            printf("[func_80073BC4] call #%d: a0=0x%08X a1=0x%08X a2=0x%08X a3=0x%08X struct=0x%08X s0=0x%08X s1=%d (0x%08X)\n",
                   s_c3bc4_cnt, a0_val, a1_val, a2_val, a3_val, struct_ptr, s0_val, (int)s1_val, s1_val);
            fflush(stdout);
        }
        return 0; /* Let compiled code execute */
    }
    ...
    switch (addr) {
        ...
        case 0x8008D60Cu: {
            ...
            /* Write to raw pad buffer */
            g_ram[0x12F9C8] = 0;
            g_ram[0x12F9C9] = 0x41;
            g_ram[0x12F9CA] = pad & 0xFF;
            g_ram[0x12F9CB] = (pad >> 8) & 0xFF;

            cpu->v0 = 0; // Ensure successful return status (non-negative)
            return 1; /* Skip native code to prevent SIO timeout clearing the buffer */
        }
        
        case 0x8008CAF0u: {
            cpu->v0 = 1; // 1 = PadStateConnected
            return 1;
        }
        case 0x8008CBB0u: {
            cpu->v0 = 2; // 2 = standard controller mode/info
            return 1;
        }
        ...
        case 0x80016940u: {
            ...
            if (s_ff <= 10) { printf("[TRACE] FUN_80016940 call %u\n", s_ff); fflush(stdout); }
            break;
        }

        /* FUN_800804F4 — FMV/MDEC display loop. Returns v0: 0 = frame done
        ...
        case 0x80028D70u: {
            ...
            if (cpu->v0 != 0) {
                printf("[PAD-READ] 80028D70 returns active-high: 0x%04X, raw al=0x%04X\n", cpu->v0, al);
                fflush(stdout);
            }
            return 1;
        }
    ...
    if (addr == 0x800809FCu || addr == 0x8007DD04u || addr == 0x8007FD00u) {
        ...
        while (limit-- > 0) {
            ...
            if (!hare_ended && ptr == hare) {
                printf("[DrawOTag] CYCLE DETECTED! ptr=0x%08X hare=0x%08X at entry=%u\n", ptr, hare, g_dt_stats.ot_entries);
                fflush(stdout);
                break;
            }
        }
        ...
        printf("[DrawOTag] #%d: a0=0x%08X %d GPU words entries=%u nonempty=%u f%u\n",
               s_drawtag, cpu->a0, g_dt_stats.total_words, g_dt_stats.ot_entries, g_dt_stats.ot_nonempty, g_ps1_frame);
        fflush(stdout);
    ...
    if (addr == 0x8007FFD4u || addr == 0x8007F444u) {
        ...
        if (phys < 0x200000u && count > 0u && count <= 8192u) {
            ...
            if (++s_ctag <= 10) {
                printf("[_otc/ClearOTagR] #%u base=0x%08X count=%u addr=0x%08X\n", s_ctag, ot_base, count, addr);
                fflush(stdout);
            }
        }
    ...
    if (addr == 0x8001EFE8u) {
        ...
        if (!s_fmv_init && g_scratch[0x1D3] == 0) {
            fmv_player_seek(g_cdrom_lba, NULL);
            s_fmv_init = 1;
...
void psx_set_pad1(uint16_t buttons) {
    ...
    /* Write to raw pad buffer for main thread func_80019304 */
    g_ram[0x12F9C8] = 0;      // 0 = connected
    g_ram[0x12F9C9] = 0x41;   // 0x41 = standard digital controller
    g_ram[0x12F9CA] = (uint8_t)(active_low & 0xFF);
    g_ram[0x12F9CB] = (uint8_t)(active_low >> 8);
}
