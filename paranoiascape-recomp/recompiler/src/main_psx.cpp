...
int main(int argc, char** argv) {
    ...
    // TMD renderer (func_8006ACF4) split-function targets:
    // The recompiler splits func_8006ACF4 into two pieces. These mid-function
    // addresses are jump targets between the pieces. Without dispatch table
    // entries, call_by_address falls to the MIPS interpreter which corrupts
    // the caller's callee-saved registers.
    analyzer.add_forced_entry(0x8006B924u);  // primitive advance (v0-=v1, s0+=v0<<2) + link check
    analyzer.add_forced_entry(0x8006B458u);  // loop back to piece 1 inner parsing
    analyzer.add_forced_entry(0x8006BEB8u);  // epilogue (restore s0-s7, sp, jr ra)
    // 0x80012E2C: Paranoiascape entrypoint (BSS memory clearing loop)
    analyzer.add_forced_entry(0x80012E2Cu);
    ...
    if (extra_funcs_path) {
        ...
        if (ef.is_open()) {
            ...
            while (std::getline(ef, line)) {
                ...
                if (addr >= 0x8000F000u && addr < 0x80200000u) {
    ...
    std::string output_filename = "generated/" + exe_stem + "_full.c";
    // Check for positional output filename (skip any --flag arguments)
    for (int i = 2; i < argc; i++) {
        if (std::string(argv[i]).substr(0, 2) != "--") {
            output_filename = argv[i];
            break;
        }
        // Skip --extra-funcs value
        if (std::string(argv[i]) == "--extra-funcs" && i + 1 < argc) {
    ...
    std::vector<PSXRecomp::Function> final_functions;
    std::string full_c_code = codegen.generate_file(analysis_result.functions, all_cfgs, &final_functions);
    ...
    // Uses the post-split function list from generate_file() so both files
    // reference exactly the same set of functions.
    {
        ...
        for (const auto& func : final_functions) {
