                ...
                if (block.exit_instr.type == ControlFlowType::Branch) {
                    ...
                    uint32_t table_count = 0;
                    {
                        ...
                        for (int back = 1; back <= 40; back++) {
                            ...
                            // SLTIU — table entry count (immediate)
                            if (s_op == 0x0B && table_count == 0) {
                                table_count = si & 0xFFFF;
                                continue;
                            }
                            // SLTU — R-type bounds check: sltu $rd, $cmp_reg, $idx_reg
                            // The count was loaded into $cmp_reg via ORI/ADDIU from $zero.
                            // Trace backward to find the immediate load.
                            if (s_op == 0x00 && s_fn == 0x2B && table_count == 0) {
                                uint32_t cmp_reg = s_rs;  // register holding the count
                                for (int back2 = back + 1; back2 <= 50; back2++) {
                                    uint32_t offset2 = (uint32_t)(back2 * 4);
                                    if (offset2 > block.exit_instr.address) break;  // underflow guard
                                    uint32_t sa2 = block.exit_instr.address - offset2;
                                    if (sa2 < cfg.function_start) break;
                                    auto sopt2 = exe_.read_word(sa2);
                                    if (!sopt2) break;
                                    uint32_t si2   = *sopt2;
                                    uint32_t s2_op = (si2 >> 26) & 0x3F;
                                    uint32_t s2_rs = (si2 >> 21) & 0x1F;
                                    uint32_t s2_rt = (si2 >> 16) & 0x1F;
                                    // ORI $cmp_reg, $zero, imm  (loads unsigned count)
                                    if (s2_op == 0x0D && s2_rs == 0 && s2_rt == cmp_reg) {
                                        table_count = si2 & 0xFFFF;
                                        break;
                                    }
                                    // ADDIU $cmp_reg, $zero, imm  (loads signed count)
                                    if (s2_op == 0x09 && s2_rs == 0 && s2_rt == cmp_reg) {
                                        table_count = (uint32_t)(int32_t)(int16_t)(si2 & 0xFFFF);
                                        break;
                                    }
                                }
                                continue;
                        ...
                        if (found_lui && table_count > 0 && table_count < 1024) {
                            uint32_t base = lui_val +
                                (found_addiu[0] ? (uint32_t)(int32_t)addiu_val[0] : 0u);
                            table_base = base + (uint32_t)lw_offset;
                            fmt::print("  Found jump table at 0x{:08X} with {} entries for jr at 0x{:08X}\n", table_base, table_count, block.exit_instr.address); fflush(stdout);
                        }
                    }
                    ...
                    if (table_base != 0 && table_count > 0) {
                        ...
                        if (!targets.empty()) {
                            ...
                            if (func.has_prologue && func.stack_frame_size > 0) {
                                ss << config_.indent << "    default: call_by_address(cpu, " << reg_name(jr_rs) << "); "
                                   << fmt::format("cpu->sp = cpu->sp + {}; return;\n", func.stack_frame_size);
                            } else {
                    ...
                    if (!emitted_switch) {
                        ...
                        if (func.has_prologue && func.stack_frame_size > 0) {
                            ss << config_.indent << fmt::format("cpu->sp = cpu->sp + {};\n", func.stack_frame_size);
                        }
...
GeneratedFunction CodeGenerator::generate_function(
    ...
    for (uint32_t block_addr : cfg.block_order) {
        ...
        body_ss << "\n" << translate_basic_block(block, cfg, func);
    }
...
std::vector<GeneratedFunction> CodeGenerator::generate_all_functions(
    ...
    for (const Function& func : functions) {
        ...
        if (cfgs.count(func.start_addr) == 0) {
            // No CFG for this function (e.g. forced entry that overlaps an existing
            // function boundary).  Emit a thin stub that defers to the runtime
            // override table and, if not overridden, falls through to the MIPS
            // interpreter so the dispatch table stays consistent.
            GeneratedFunction stub;
            stub.function_name = func.name;
            stub.signature = fmt::format("void {}(CPUState* cpu)", func.name);
            stub.body = fmt::format(
...
std::string CodeGenerator::generate_file(
    const std::vector<Function>& functions,
    const std::map<uint32_t, ControlFlowGraph>& cfgs,
    std::vector<Function>* out_final_functions) {
    ...
    ss << "    return (rt_value & ~mask) | (word & mask);\n";
    ss << "}\n";
    ss << "\n";
    ...
    ss << "    uint32_t shift = (addr & 3u) * 8u;\n";
    ss << "    uint32_t mask  = 0xFFFFFFFFu << shift;\n";
    ss << "    cpu->write_word(aligned_addr, (existing & ~mask) | (rt_value & mask));\n";
    ss << "}\n";
    ss << "\n";
    ...
    // ---- Pre-pass: DISABLED ----
    // The pre-pass splits functions at mid-function branch targets, but this
    // creates broken trampolines: the fallthrough between split pieces becomes
    // a function CALL (not a goto), causing infinite loops when the called
    // piece returns to the original caller (e.g. func_800A2B04 ↔ func_800A2B14).
    // Jump table targets are already handled by --extra-funcs.
    if (false) {
        ...
        for (int pass = 0; pass < 3; pass++) {
            ...
            for (auto& f : functions_mut) {
                if (affected.count(f.start_addr)) {
                    if (f.end_addr <= f.start_addr || f.size == 0) continue;
                    auto new_cfg = cfg_analyzer.analyze_function(f);
                    cfgs_mut.erase(f.start_addr);
                    cfgs_mut.emplace(f.start_addr, std::move(new_cfg));
    ...
    // Deduplicate functions_mut by start_addr.  The pre-pass can create entries
    // at addresses that already exist as forced entries, resulting in duplicates.
    // Keep the first occurrence (which has the correct size from the pre-pass).
    {
        std::set<uint32_t> seen;
        auto new_end = std::remove_if(functions_mut.begin(), functions_mut.end(),
            [&seen](const Function& f) { return !seen.insert(f.start_addr).second; });
        if (new_end != functions_mut.end()) {
            fmt::print("Dedup: removed {} duplicate function entries\n",
                       std::distance(new_end, functions_mut.end()));
            functions_mut.erase(new_end, functions_mut.end());
        }
    }
    ...
    // Export the actually-generated function list so the dispatch table stays
    // in perfect sync with full.c (functions_mut may contain duplicates or
    // entries that were absorbed during the pre-pass).
    if (out_final_functions) {
        out_final_functions->clear();
        for (const auto& gf : gen_funcs) {
            // Parse address from function name "func_XXXXXXXX"
            uint32_t addr = (uint32_t)std::strtoul(
                gf.function_name.c_str() + 5, nullptr, 16);
            Function f;
            f.start_addr = addr;
            f.name = gf.function_name;
            out_final_functions->push_back(f);
        }
    }
