...
public:
    ...
    // If out_final_functions is non-null, populates it with the post-split
    // function list (the same list used to generate full.c).  Use this for
    // the dispatch table so both files stay in sync.
    std::string generate_file(
        const std::vector<Function>& functions,
        const std::map<uint32_t, ControlFlowGraph>& cfgs,
        std::vector<Function>* out_final_functions = nullptr);
